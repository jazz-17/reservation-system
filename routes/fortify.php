...
$fortifyMiddleware = array_values(array_filter(
    config('fortify.middleware', ['web']),
    fn (string $middleware): bool => $middleware !== 'web',
));

Route::group([
    'domain' => config('fortify.domain', null),
    'prefix' => config('fortify.prefix', ''),
], function () use ($fortifyMiddleware) {
    Route::group(['middleware' => $fortifyMiddleware], function () {
        ...
        $passwordEmailLimiter = config('fortify.limiters.password-email');
        ...
        if (Features::enabled(Features::resetPasswords())) {
            ...
            Route::post(RoutePath::for('password.email', '/forgot-password'), [PasswordResetLinkController::class, 'store'])
                ->middleware(array_filter([
                    'guest:'.config('fortify.guard'),
                    $passwordEmailLimiter ? 'throttle:'.$passwordEmailLimiter : null,
                ]))
                ->name('password.email');
